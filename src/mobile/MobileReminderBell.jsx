...
// (9am on a working day) for it via the OS's native "add event" flow. `id`
...
// there's no backend to sync it against). The date is staggered by how many
// other reminders are already set, so setting several in one sitting spreads
// them across different days instead of stacking them all onto tomorrow.
export default function MobileReminderBell({ id, title, description }) {
  ...
  const handleClick = (e) => {
    e.stopPropagation();
    triggerCalendarReminder({ title, description, date: reminderDate });
  ...
  return (
    <button type="button" onClick={handleClick}
      aria-label={isSet ? "Reminder set" : `Set a reminder for ${reminderDate.toLocaleDateString("en-GB",{weekday:"short",day:"numeric",month:"short"})}`}
      title={isSet ? "Reminder set" : `Remind me ${reminderDate.toLocaleDateString("en-GB",{weekday:"short",day:"numeric",month:"short"})}`}
      style={{
